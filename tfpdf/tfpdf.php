...
function Header()
{
	...
	if (empty($_REQUEST["itype"])) {$_REQUEST["itype"] = NULL;}

	if ($orientation == "L")
	{	if (substr($_SERVER['HTTP_ACCEPT_LANGUAGE'],0,5) == "en-US")
		{	if (substr(basename($_SERVER["PHP_SELF"]),0,5) == "arbre" AND $_REQUEST["itype"] != "liste")
		         {$this->Image ("themes/bord_LE_paysage_tree.png", 0, 0, 279, 216);}
			else {$this->Image ("themes/bord_LE_paysage.png", 0, 0, 279, 216);}
		} else
		{	if (substr(basename($_SERVER["PHP_SELF"]),0,5) == "arbre" AND $_REQUEST["itype"] != "liste") 
		         {$this->Image ("themes/bord_A4_paysage_tree.png", 0, 0, 297, 210);}
			else {$this->Image ("themes/bord_A4_paysage.png", 0, 0, 297, 210);}
		}
	} else
	{	if (substr($_SERVER['HTTP_ACCEPT_LANGUAGE'],0,5) == "en-US")
		{	if (substr(basename($_SERVER["PHP_SELF"]),0,5) == "arbre" AND $_REQUEST["itype"] != "liste") 
		         {$this->Image ("themes/bord_LE_portrait_tree.png", 0, 0, 216, 279);}
			else {$this->Image ("themes/bord_LE_portrait.png", 0, 0, 216, 279);}
		} else
		{	if (substr(basename($_SERVER["PHP_SELF"]),0,5) == "arbre" AND $_REQUEST["itype"] != "liste") 
		         {$this->Image ("themes/bord_A4_portrait_tree.png", 0, 0, 210, 297);}
			else {$this->Image ("themes/bord_A4_portrait.png", 0, 0, 210, 297);}
		}
	}
	...
	$this->cell (0,4,$entete,0,1,"C");
	$this->SetFont('DejaVu','B',12);
	if (isset($entete1))
	{ $this->cell (0,4,trim($entete1),0,1,"C");
	}
	...
	{	$this->SetFillColor(255);
		...
		$this->Cell(0,4,"","0",1);
		$this->Cell($LargCol[0]              ,4,'',0,0,"C",1);
		$this->Cell($LargCol[1]              ,4,$got_tag["NAME"],"TBLR",0,"C",1);
		$this->Cell($LargCol[2] + $LargCol[3],4,$got_tag["BIRT"],"TBLR",0,"C",1);
		$this->Cell($LargCol[4] + $LargCol[5],4,$got_tag["MARR"],"TBLR",0,"C",1);
		$this->Cell($LargCol[6] + $LargCol[7],4,$got_tag["DEAT"],"TBLR",0,"C",1);
		$this->Cell(0,4,"",0,1);
	}
}
...
function Footer()
{
	...
	setlocale(LC_TIME, $_REQUEST["lang"]);
	$this->SetXY(16,-20);
	$this->SetFont('Arial','',7);
	$this->SetTextColor (0,0,0);
	$this->cell(0,3,date("d F Y"));
...
// patch Damien : compatiblity PHP 8 null is not authorized
	if ($s !== NULL) 
